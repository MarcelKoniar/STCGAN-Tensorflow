...
def load_image(addr, is_mask=False):
    ...
    if img is None:
        return None
    img = cv2.resize(np.float32(img), (IMAGE_SIZE, IMAGE_SIZE), interpolation=cv2.INTER_CUBIC)
    #if(is_mask==True):
    #    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    #else:
    #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
...
def read_from_tfrecord_triple_img(reader, filenames):
    ...
    shadow = tf.decode_raw(tfrecord_features['shadow'], tf.float32)
    shadow_mask    = tf.decode_raw(tfrecord_features['shadow_mask'],    tf.float32)
    shadow_free    = tf.decode_raw(tfrecord_features['shadow_free'],    tf.float32)
    ...
    if vis:
        with tf.Session() as sess:
            ...
            for i in tf.python_io.tf_record_iterator(tfrecord_file):
                ...
                #pilimg.show()
                iminput=decode_image(s)
                cv2.imshow("s",iminput)
                iminput=decode_image(m)
                cv2.imshow('m',iminput)
                iminput=decode_image(f)
                cv2.imshow('f',iminput)
